...
- 最小化电力消耗 （ 关闭所有 冗余并发。精简浓缩 规则数量。并按减少规则命中前的比对次数。将日常场景中，高命中的规则放在最前部，避免出现 对比了几万条 才发现是 域名是🇨🇳国内IP 要直连。 ）
- 支持 切换IP归属地显示（ 覆盖国内主流APP ）
- 支持 黑白名单模式 实时切换 （ 全程 无需切换规则文件 ）
- 支持 按地理位置就近 分配节点
- 保证访问IP稳定 （ 单一节点被墙，也不会乱跳IP ）
- 双外部规则源 互为备份 （哪怕任意一个外部规则源 更新慢，也最大化避免手动调整）
...
- **内置多外部规则源**：
    - 同时内置两组 互为备份的 外部规则源：
        ...
        + 两组规则 = 90%重叠：即，哪怕任意一组 维护更新慢，另外一组也能补充上。
...
如何将本模版默认是从白名单模式。如果像切换拥有一个只支持黑名单模式的模版，那只需要在配置模版中，检索 “ BlackList ” 关键词，注释掉这些关键词包裹住的行，你就会得到一个 黑名单模版。

- 当然，我个人建议，如果想省心，还是要 1TB以上月流量的 高质量节点 + 白名单模式。这样才能做到足够省心。毕竟黑名单最大的问题，就是可能会三天两头 补充规则，不胜其烦。花钱 买省心，就像手机买iphone 购物去山姆 一样。
- 关于 如何避坑 底质量节点，请看本页面的末尾
...
## 使用建议
 ...
 - 总之：
     - 规则数量，不是越多越好 ！！！！iPhone 等手机使用，需求的优先级排序 一定是：
         1. 省电  
         2. 隐私性  
         3. 准确度高（❌错误规则） 
         4. 另一套外部规则源（作为备份） 
...
## 如何魔改本模版，从而做到 进一步省电？
   ...
   - 本套规则中， 为什么没有使用 Geosite ：CN ？
       + 因为  Geosite ：CN 里面包含上千条 错误的规则，这些规则背后指向了境外IP，却被标识成中国域名。最离谱的是，居然把TikTok的所有域名，都标记为了中国域名 🤣简直是错误大王。实际上他们的维护者，只需要让Chatgpt写一个自动测试域名是否GEOIP是中国IP的脚本，每周跑一次，就能自动化排查所有错误。可是压根没人维护。本模版的中国域名，使用的是 blackmatrix7/ios_rule_script 中的 China分流规则 （https://github.com/blackmatrix7/ios_rule_script/tree/master/rule/Clash/China ）。这个规则集只有3500条，但是配合GEOIP：CN后，完全够用。对于此规则中错误部分，我也做了修复规则PreRepairChina （使用ChatGPT Pro Agent 自动排查China规则集中的非中国域名），以力求完美。
       + 总之，强烈不建议使用 Geosite ：CN 。
...
2. 尽量不要选择 ：“海外华人、🇨🇳 中国人” 的云服务公司 。坑非常多，例举几点：
    ...
    - 国产厂商，各种虚标。
        - 包括线路稳定性（可连通性），也跟老外的厂商无法对比，比如像MoeCloud（ https://lite.moe/ ），号称自己英国线路强无敌。但实测经常会间歇性断联。频繁的时候，一周断好两三次，一次好几个小时。
    - 国产厂商，相当多 吃相难看 + 套路满满。
    ...
    - 国产厂商，没有契约精神。
        - 在很多电报群中，还有翻友袒露，像saltyfish （ https://portal.saltyfish.io/ ），还会应为你提问态度不好或在工单中辱骂客服，直接封你号，哪怕你的VPS没到期，封号后也不退款。更不会因为，故意限速你的下载，而给你退款。一副 “钱我已经骗到手，你能拿我怎么样” 的骗子嘴脸 🤣 。毫无契约精神。我相信这不是saltyfish一家有的问题。其他 一人公司的🇨🇳厂商，不胜枚举。
...
4. 落地节点，要测试IP的干净程度，最好是双ISP 家庭宽带IP 或者 移动网络IP。但还要检测其国际出口 或落地国的出口是否足够大。大部分这种IP的节点，出口都很小，50M或者200M顶天了。而且。如果真的是 “美国的车库级机房” 你还可能会遭遇 频繁断流。坑比较多。
...
2. 单一节点，除了准备多IP，最好也能通过gost多设置几个影子端口（端口转发）。因为现在GFW封禁都是精准封禁，即便被封，往往都是先从封禁端口开始，这会备用端口就能派上用场了。以防止境外节点失联后，无节点可应急的情况。
