...
为了降低维护成本！ 在 🇨🇳中国，有相当多的IOT设备，未来都会OTA升级到内置反诈插件（目前国产路由器、国产光猫已经内置反诈插件了），届时 只要反诈插件 跑几个简单的 预设名单，就能拿到你在路由器上配置的 VPN节点IP，然后你的所有VPN IP都会被阻断，你连后手都没有。🤣🤣 纯纯的 0风险意识。

什么样的IOT设备能实现上述效果？比如联通赠送的家庭摄像头、国产智能电视机、扫地机器人、智能中继、甚至你家智能冰箱。这些设备都可以实现上述效果。风险极大。尤其像智能电视这种，非常容易被病毒程序攻击，其实很多人家里的智能电视，都已经被入侵，在必要时会成为DDOS源，在平时，你家电视也可能会间歇性的发送垃圾邮件（消耗流量极低，你在路由器流量监控中根本看不出来有问题）。这些都是99.999%的人观察不到的。这些病毒，在发送垃圾邮件的过程中，如果匹配到了你部署在OpenClash路由器上的的规则，如果使用了你的VPN节点发垃圾邮件，会导致你的节点瞬间被封禁。即便你是订阅节点不怕被封，那你也可能会频繁切中转线路，体验极差。再进一步，如果这些设备，升级内置反诈插件，成为GFW部署在你家局域网内的探测器，那更会让你部署在路由器上的所有VPN节点同时报废 。所以，强烈 不不不不不不不建议，把所有节点和规则，都配置到家庭总出口的路由器上。
