...
<br>
 ...
 -  本模版使用 ，**DNS白名单解析 + IP白名单分流**  = 几乎99.99%覆盖 🇨🇳 == 0绕路。
 
     -  DNS白名单解析：
         -  为了让🇨🇳中国网站，只会解析到🇨🇳国内IP。本模版 在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。让国内网站的域名，直连国内DNS，解析到国内IP地址。
     -  IP白名单分流：
     ...
     - 如果 仍旧满足不了你的需求？
         - 可以将你认为发生绕路的 🇨🇳 国内域名，加入到本模版的DNS分流策略中：即，在 “ nameserver-policy ” 字段中。可秒修复 绕路问题 
