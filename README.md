...
# 关于 Geosite ：CN ，给 Clash、Stash 这些软件开发者的建议

想一劳永逸解决 GeoSite的问题，必须Clash这样的软件，能够支持，用户能一键导出 自己出过去一两年内的域名解析请求记录。然后， 支持 一键测试，用本国DNS，测试这些域名 是否指向国内IP，然后为用户生成自己本地的Geosite:CN。做到完全贴合用户使用习惯，无浪费规则 + 又极致省电。

而不是让用户，在VPN上，时时刻刻都挂着 10多万条规则的ChinaMax规则。使得每一次网络请求，都比对十几万次，才能命中。这纯脆是0价值费电。

当然，用户也可以将自己的本地的Geosite:CN分享给社区，从而让社区的规则集合，可以集腋成裘。

<br>

多说一句。从使用体验上来看， 像geosite:cn 这种基于泛LBS概念的分流规则， 给 ”dns解析请求“ 分流的价值，至少十倍于 给其他类型的网络请求。
