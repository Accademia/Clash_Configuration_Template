...
- **0 DNS泄漏** （ 禁用Fallback DNS + 禁用Fake IP 。只使用 “DNS分流策略” 实现 0 DNS泄漏。让🇨🇳国内域名 只发送给 🇨🇳 国内DNS。让海外域名，只通过 海外VPN节点 转发给海外dns进行解析，不会发送给国内DNS服务器。且两者 全程使用DOH加密访问！ ）
- **满血版DNS解析** （ 所有域名解析，都 “不添加” no-resolve限制 ）
- 访问🇨🇳国内网站时 **几乎0绕路海外** （ 直连规则 = geosite:cn + geoip:cn ） 
...
不会❕❕❕
 ...
 - 本模版使用了 geosite:cn 对DNS域名解析请求 进行分流，又使用了geoip:cn 对域名解析后的IP进行分流。从而确保几乎是 99% 情况下，访问🇨🇳网站时 ，不会绕路到国外。
 
 - 而对于，不包含在geosite:cn中的冷门网站 ？ 您放心，越是冷门，100%越是用不起海外CDN（ 穷 + 核心受众在国内 ），或者压根不屑于用海外CDN（比如 医院 大学 政府）。在这种情况下，你在外国DNS请求域名解析，一样返回的是中国IP。只要是🇨🇳中国IP，就会通过geoip:cn分流到 “直连”。可以说， **geosite:cn + geoip:cn**  = 几乎99.99%覆盖 🇨🇳 == 0绕路。
 
 -  本模版，同时还给了 “微软、苹果、索尼、任天堂” 特别预设。对于海外公司，只默认这四个大厂，是直连模式（但是，其AI服务除外）。以便于， 避免 系统升级、软件升级、游戏升级 时，把VPN流量耗光 ❕❕❕

 - 使用白名单模式，更不会导致上网时，上网变卡。
