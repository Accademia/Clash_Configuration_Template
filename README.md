...
- 👍👍👍 如果有人愿意将本模版 修改为 《 0手工配置 就可无脑导入的 覆写、订阅 版本》，也欢迎通过issue传递给我。我可以 参与整理。
  > 要求 ：
  > - 增量代码，要有必要注释  
  > - 不破环 原有代码风格（ 如：必须列对齐。对引号的处理。变量名一致。等 ） 
  > - 保留所有存量功能（ 如 ：通过代理链稳定IP的功能 ）  
  > - 方便通过 Beyond Compare 进行逐行对比（ 便于维护 ）
...
### 以下两组规则，是在上述 [通用模版] 基础上，删除了 “Stash兼容” 
...
#### 以上两套模版：《3个桌面端模版》vs 《3个移动端模版》

- 两者唯一区别，就是是否启用了 ChinaMax 和 GFWlist 。 

   > 移动端 , 关闭了 这两个规则。因为。这两个规则启用后，规则数量 增长10倍 ❕ 从1.2万条 增长到 12万条 ❕ 功耗爆炸，移动端续航 直接 ➗ 2
...
### 每个表格中，三套子模版 之间 ，选哪一个？
 ...
 - #### 【手机、平板、路由器】 ： 首选 ✅ [ClashMeta模版]-[移动端]-02 ✅
    ...
    > 对比：“模版 01” 虽然更全面，但是在移动端会极大消耗电力，续航➗2。
    
    > 优势：“模版 02” 已经可以屏蔽90%的广告，并且电力消耗 与 “模版 03”（无反广告 无反隐私泄漏）基本持平。
