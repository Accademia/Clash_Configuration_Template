...
建议步骤（最佳实践）：
  ...
  - 第四步 ，导入到 Clash Meta 或 Stash 前的必备工作 
      ...
      - 如果导入到 Stash for iPhone 客户端 ， 在源代码中查找：
          
          - ClashMeta_Only 包裹着的代码 ，关闭 ❌
          
          - Stash_Compatible 包裹住的代码，启用 ✅

          - ⚠️ 对于 Stash for iPhone，强烈建议做上述修改。因为，默认配置 = 只能做到stash客户端不报错 + 翻墙基本功能正常工作。但默认配置下，链式代理 在stash客户端上，会 退化为 （与落地机）节点直连。必须做完上述改动，才能让链式代理在stash上生效！当然，如果你无需代理链功能，则这一步可以省略。
            
  - 第五步（非必要），如果 你嫌 VPN流量贵 ，可以切换到 “仅支持” 黑名单模式
