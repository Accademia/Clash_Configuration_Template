...
本规则在iPhone、MacBook上，完全兼容 Stash （ https://stash.wiki/ ）

在适配Stash的过程中，本模版是如何做到省电的 ？ 主要遵循一下原则：
...
## 关于在 本模版 与 Stash不兼容的地方

以下三个问题，是本模版与Stash无法兼容的地方 （ 但在Clash Meta核心上完美兼容 ）


1. Stash 无法分流DNS查询到指定的DNS服务器（即 ，无法分流中国网站（Geostie:CN）的DNS查询 到 中国DNS服务器）
    - 由于并不能在DNS的nameserver-policy:中支持Geostie:CN，所以导致了，在本模版启用了follow-rule后。所有域名查询全走境外DNS查询，从而导致如京东养车等APP会返回海外IP。向开发者多次反馈后，开发者压根不回复 不响应 （不知道怎么想的） 。而Clash Verga rec客户端（Clash Meta核心），不会有这个问题。
...
2. Stash 的链式代理，不支持UDP流量
   而诸如 苹果智能，等AI软件，会大量使用UDP流量，而且这些工具，对IP的合规性还有极高要求，很多机房IP都会间歇性封禁。必须使用拥有home ip的节点作为落地节点。且往往这些节点的速度都不高，不适合直连。所以，对于此类APP，支持UDP流量的链式代理是刚需。而目前Stash并不能支持这个。向开发者多次反馈后，开发者压根不回复 不响应 （不知道怎么想的）
   
3. Stash 即无法 “关闭DNS的IPv6解析” ，也无做到 “全透明转发IPv6流量”。
    
    这导致了，在关闭Stash的FakeIP功能后，如果你在Stash中 “关闭” IPv6路由后，那么如下软件，都会出现出现了连接不上的情况：
    - 在 Grok APP中文字提问
    - 在 苹果智能设置页面 登录ChatGPT账号 
    
    这还导致了，在关闭Stash的FakeIP功能后，如果你在Stash中 “开启” IPv6路由后，那么如下软件，都会出现出现了连接不上的情况：
    - 在 homekit中 观看 局域网摄像头
   Stash的功能缺失，使得上述两者，相互互斥。而在Clash Meta核心的工具上，是没这种情况。
