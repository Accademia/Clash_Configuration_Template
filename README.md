...
<br>
 ...
 - 本模版，在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。即，只有在白名单内的中国网站，才会向🇨🇳中国国内的DNS 发起域名解析请求。不在白名单内的网站，一律通过 VPN节点转发给 境外DNS进行解析。且两者全程都是DOH加密格式。从而保证100%不会发生DNS泄漏！
 
 - 全程禁用以下功能，以获得 “满血版DNS解析” 能力：
     - ❌ Fake IP   
     - ❌ no-resolve
     - ❌ Fallback DNS
