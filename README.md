...
## 如何魔改本模版，从而做到 进一步省电？
   ...
   - 对于模版文件内，同时有 Ruleset规则 和 Geostie 规则的 分流选项上，删除Ruleset规则（ 并且删除其对应的rule-providers ）。这样可以将，外部规则数量➗2，同时，几乎不会降低分流能力。从而进步一省电。也就是说，这两者是互为冗余部署的。
