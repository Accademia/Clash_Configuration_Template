...
#  本模版，会不会绕路（DNS解析到国外IP）？

不会 ❕  

  - 通过 DNS分流策略，中国域名 只向 中国DNS 请求解析。从而获得国内CDN IP。实现 0境外绕路。

为了实现上述目标。本模版的DNS分流策略中，并没有 geosite:cn 规则集，因为这个规则集的精度太低。错误和冗余规则，高达2000多条，占其集合内规则总数量的29%。 会导致错误的DNS分流。
