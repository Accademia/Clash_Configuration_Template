...
## 想进一步省电，如何魔改本规则
   ...
   - 在同时有 Ruleset规则 和 Geostie 规则的 分流选项上，删除Ruleset规则（ 并且删除其对应的rule-providers ）。这样可以将外部规则数量减少到1000个以下。从而进步一省电。
     
   - 本套规则中，在同一个分流策略上，同时安排了 Ruleset + Geostie 。 是通过冗余规则来避免因某个规则库停止更新（或更新慢），而导致需要频繁手动调整yaml规则。尽量做到一次书写，后期0维护。

.


##  如何使用这个模版 ？ 如何 将节点 添加进 这个模版 ？
