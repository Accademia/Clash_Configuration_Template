...
如何启用 本模版中的 此功能？
  ...
  - 在 分流规则（rule） 和 dns分流策略（nameserver-policy）之间，优先选择 dns 分流策略，激活本远程规则集。即，在dns分流阶段，通过中国分流规则，如果能获取🇨🇳 中国ip，已经可以在后续流量分流规则（rule）中，通过 geoip:cn 分流中国流量了。即，在分流规则（rule）上其实已经无需 应用ChinaMax规则 进行二次分流了。
