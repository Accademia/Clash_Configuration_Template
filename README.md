...
- **完全隐秘行踪**：
    ...
    - 即，除 “苹果、微软、索尼、任天堂” 这四个超级大厂（非AI服务）外，所有境外流量（包括DNS查询），在默认配置下，均通过VPN节点代理出境。
...
本模版的 白名单模式 ， 会不会 “绕路”，导致消光VPN流量 ？

 - 不会！ 本模版使用了 （魔改后的）geosite:cn 对DNS域名解析请求 进行分流，又使用了geoip:cn 对域名解析后的IP进行分流，从而确保几乎是100%国内的网络访问 ，只会请求国内的CDN服务器。
 
 - 而对于，不包含在geosite:cn中的冷门网站？您放心，越是冷门，丫越是用不起海外CDN（ 穷 + 核心受众在国内 ），或者压根不屑于用海外CDN（比如 医院 大学 政府）。所以，使用 **geosite:cn + geoip:cn**  = 几乎100%覆盖 🇨🇳 == 0绕路。
 ...
 -  总之， 白名单 ≠ 几十万中国规则。为了使用频度极低的域名，增加几万条或十几万条规则，完全 = 天选韭菜 ， 毫无成本意识 。在移动端是不可接受的。

<br>

本模版的 白名单模式 ，会不会 导致 系统升级（iOS/Windows）、AppStore升级 时，把VPN流量耗光？

 - 不会！ 本模版给了 “微软、苹果、索尼、任天堂” 特别预设。海外公司，只默认这四个大厂，是直连模式（但是，其AI服务除外）
