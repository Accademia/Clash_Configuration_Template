...
本模版开箱即用，你只需要 在源代码中，找到下面 6处。将你的 VPN节点 S配置到下面六处即可。

  - 如果你只有一个VPS，你可以将六处都配置成同一个VPN节点。也不影响使用。
  
  - 首次验证时，强烈建议，只使用一个VPN节点复制六份。即，强烈建议：
      - 先验证能否跑通。再验证分流（再将不同国家VPN节点配置上去）

  - 本模版，只适配了 自建线路（自购VPS）的情况。如果是你的VPN节点是订阅的， 建议使用AI帮你完成添加：把 模版、本说明文件、你的订阅VPN节点的配置文件一起扔给AI，让AI帮你完成，并且要告诉AI，不要修改本模版任何规则部分，只做 “增量更新” ，“只更新” 节点订阅。
 
    - 如果你要使用AI进行修改，强烈建议，需要用Beyond Compare这样的源代码比对软件，对AI的修改结果进行比对。已防止AI不听指挥。对AI生产的结果，必须二次验证，不然100%被坑。
  
  - 再次强调，本模版 =  自用级验证 + 24小时常驻后台 ， 100%不会出问题。⚠️⚠️⚠️ 改出问题，99.999% 只可能是你自己的问题 ⚠️⚠️⚠️ 不是模版问题，不要发issue 去问AI。本人不提供任何一般性问题的解答。只，处理BUG、升级功能、收集需求。 
