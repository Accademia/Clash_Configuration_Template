...
建议步骤（最佳实践）：
  ...
  - 第四步（非必要），逐一检查，如下配置，是否有 微调需求 
      ...
      - 选配点 1 ：是否切换到 ：只支持 Clash Meta 模式❓ ｜ 启用方法，在源代码中查找：
          ...
          - ⚠️ 对于 Clash Meta核心的客户端 ，强烈建议做上述修改。因为，默认配置 = 只会避免 🇨🇳中国流量 绕路 海外VPN，但是无法避免 海外流量 发生 全球绕路，即，日本VPN节点 会请求 美国CDN的数据。必须做完上述改动，才能在Clash Meta客户端上避免出现全球绕路！
      ...
      - 选配点 2 ：是否切换到 ：只支持 Stash 模式❓ ｜ 启用方法，在源代码中查找：
          ...
          - ⚠️ 对于 Stash for iPhone，强烈建议做上述修改。因为，默认配置 = 只能做到stash客户端不报错 + 翻墙基本功能正常工作。但默认配置下，链式代理 在stash客户端上，会 退化为节点直连（落地机）。必须做完上述改动，才能让链式代理在stash上生效！
