...
✅ 目前 本模版 = 100% 达到 🇺🇸 “美国数字居民” 级 VPN 的规格 ❕❕❕
...
<br>
 ...
 -  本模版使用 ，**DNS白名单解析 + IP白名单分流**  =  几乎99.99% 🇨🇳 国内直连 
 
     -  DNS：白名单解析
         -  为了让 🇨🇳 中国网站，只会解析到🇨🇳国内IP。通过在 “DNS分流策略” 中，使用 白名单机制（geosite:cn）。让国内网站的域名，直连国内DNS，从而解析到国内IP地址。
         
     -  IP：白名单分流
         -  为了将（上述DNS解析后的）🇨🇳国内IP地址，直连中国大陆。通过在 “分流规则” 中，使用 白名单机制（geoip:cn）。使得 访问🇨🇳国内网站 ，不会绕路到国外。
 
     - 而对于，不包含在 “DNS分流白名单” 中的冷门网站，会不会绕路 ？ 
     ...
     - 如果 仍旧满足不了你的需求？
         - 可以将你认为发生绕路的 🇨🇳 国内域名，加入到本模版的DNS分流策略中：即，在“ nameserver-policy ” 字段中 加入新域名。可秒修复 绕路问题 
 
<br>

 -  为了避免过量消耗VPN流量。本模版，同时还给了 “微软、苹果、索尼、任天堂” 特别预设。对于海外公司，只默认这四个大厂，是直连模式（但是，其AI服务除外）。以便于， 避免 系统升级、软件升级、游戏升级 时，把VPN流量耗光 ❕❕❕
...
富帅闭眼白名单就对了。没钱架优质线路的，才会选黑名单。 （ 如何避坑 劣质节点，在本文末尾 ）
...
<br>
 ...
 - 理由 3 ：白名单 = 私密性高❕ 🫥🫥🫥🫥🫥
     ...
     - 不仅仅在中国，即便是在美国。大牌上市安全公司，在零售市场主推的VPN，也都是白名单VPN。比如，像诺顿、麦咖啡 这些美国顶级杀毒软件公司，其主要营收，已经不是杀毒软件了，而是白名单VPN。核心卖点就是，基于VPN保护自己网络安全，尤其是私密性，防止被第三方深度包检测 或 防止被检测到自己的网络位置。可以认为，卖点是基于VPN加速的安全通道（或称之为 基于VPN的防火墙）
 
 - 理由 4 ：白名单 = 最省电❕
     
     - 黑名单模式，规则会膨胀到2.7w条规则。而白名单可以小于2w条规则。
...
<br>
 ...
 - 关于 0 DNS泄漏：
     ...
     - 本模版，在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。即，只有在白名单内的中国网站，才会向🇨🇳中国国内的DNS 发起域名解析请求。不在白名单内的网站，一律通过 VPN节点，转发给 境外DNS进行解析。且两者全程都是DOH加密格式。从而保证100%不会发生DNS泄漏！即，任何 🌍 国外域名，都不会向 🇨🇳 中国DNS服务器 发起域名解析请求。 并且，在上述过程中，全程禁用：
...
<br>
  ...
  - 实际上，为了防止 GFW通过 “深度包检测” 技术，监控你你的境外行为，你必须同时启用两项功能（，才能让GFW的“深度包检测” 技术 全部归零 ，让GFW在你身上完全失效：
...
这是因为，通过冗余部署，两套远程规则（Rule-Set + Geosite ），避免因某一个规则库停止更新（或更新慢），而导致，使用者频繁手动维护yaml。这样部署，更省心，尽量做到一次书写，后期0维护。
...
- 所有国产设备，都内置了 🦠🦠🦠🦠 反诈插件 🦠🦠🦠🦠，会直接导致你的翻墙节点，被实时监控。
    ...
    - 关于国产手机：
        ...
        - 2023年开始，所有国产手机都内置了 🚧 **反诈插件** 🚧 ，功能比路由器内置的反诈插件还要更强，可以直接扫描你手机内的所有APP和所有数据。翻墙软件，被归类为 **有害软件** 。理论上，你手机内所有的数据，都可以上报给GFW。隐私性 = 0.
        
        - 测试你手机有没有 反诈插件 的方法：关闭所有网络连接后，能不能修改手机名 ？ 比如把手机名改成 “8口交换机”。如果离先后 不让你改名，或者 在线情况下 因为“口交”是敏感词，也不让你改名。恭喜你，你手机100%内置反诈插件。建议只能换三星苹果等进口品牌的手机。
        
        - 总之，不要购买 ❌ 国产手机。如果你真想支持国产厂商，可以买国产手机的海外版，比如出场Google Play的小米手机，那些都是没有内置反诈插件的。相对比较安全。但任意国产手机，都会被警方解锁。唯一完全安全，无法被警方解锁的，只有iPhone。

- 用iPhone  或者 重装Windows + 避免国产扫盘类的软件（如wps、360等），是保护隐私的唯一出路。
