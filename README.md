...
### 2.  第二套模版 ： 移动端 + 白名单模版 ：专属 Clash Meta模版 
  ...
  #### 注意：以下所有模版，都是，通过对 上述《通用模版》进行进一步 裁剪、阉割，从而得到的 （ = 0添加 新代码 ）。三套模版的关系，与通用模版一一对应。只适配 Clash Meta内核 客户端。
...
#### 注意：与 通用模版比，在 兜底规则前，启用了 GFWlist规则 （7000条） ，用于在黑名单模式下，增强突破GFW的能力
