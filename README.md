...
- 避免 被APP识别出 使用了VPN（ 避免 APP的安全机制，通过识别VPN返回的假IP，识别出正在使用VPN。禁用FakeIP功能 ）
- **0 DNS泄漏** （ 让 🇨🇳 国内域名 ，只发送给 国内DNS。让 🌍 海外域名，只通过 海外VPN节点 转发给海外dns进行解析，不会同时发送给国内DNS服务器。且 两者 全程使用DOH加密 ）
- **满血版DNS解析** （ 不限制域名解析。 全程禁用 no-resolve ）
- 访问🇨🇳国内网站时 **几乎0绕路海外** （ 让中国网站 只获取🇨🇳国内IP。避免获取其国外镜像CDN的IP ） 
...
#  本模版的 “白名单模式” ， 会不会 “绕路” 导致 大量消耗VPN流量 ？或者导致打开网页变卡？
...
<br>
 ...
 -  本模版使用 ，**DNS白名单解析(geosite:cn) + IP白名单分流（geoip:cn）**  = 几乎99.99%覆盖 🇨🇳 == 0绕路。
 
     -  DNS白名单解析：
         -  为了让🇨🇳中国网站，只会解析到🇨🇳国内IP。在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。让国内网站的域名，直连国内DNS，解析道国内IP地址。
     -  IP白名单分流：
         -  在获得🇨🇳国内IP地址后，又使用了geoip:cn 对域名解析后的IP进行分流。从而确保几乎是 99% 情况下，访问🇨🇳国内网站时 ，不会绕路到国外。
 
     - 而对于，不包含在DNS分流白名单（geosite:cn）中的冷门网站，会不会绕路 ？ 
         - 您放心，越是冷门小众，100%越是用不起海外CDN（ 穷 + 核心受众在国内 ），或者压根不屑于用海外CDN（比如 医院 大学 政府）。在这种情况下，你在外国DNS请求域名解析，一样返回的是中国IP。只要是🇨🇳中国IP，就会通过geoip:cn分流到 “直连”。
    
     - 如果 仍旧满足不了你的需求？
         - 可以将你认为发生绕路的 🇨🇳 国内域名，加入到本模版的 “ nameserver-policy ” 字段中。即可秒修复 绕路问题 
 
<br>

 -  如何避免 其他，非必情况下，过量消耗VPN流量？
...
<br>
 ...
 - 理由 3 ：白名单 = 私密性高❕ 
     - 对上层网关完全隐秘 “境外行踪”。而黑名单 会被墙一览无余自己的境外足迹。要知道，在美国本土，也有大量美国人用（诺顿、麦咖啡 这些杀毒软件公司提供的）VPN，主打就是，人家愿意花钱买VPN，保护自己的私密性。
...
<br>
 ...
 - 关于 0 DNS泄漏：
     ...
     - 本模版，在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。即，只有在白名单内的中国网站，才会向🇨🇳中国国内的DNS 发起域名解析请求。不在白名单内的网站，一律通过 VPN节点转发给 境外DNS进行解析。且两者全程都是DOH加密格式。从而保证100%不会发生DNS泄漏！即，任何 🌍 国外域名，都不会向 🇨🇳 中国DNS服务器 发起域名解析请求。 并且，在上述过程中，全程禁用：
         - ❌ Fake IP   
         - ❌ Fallback DNS
