...
## 项目概述
本项目提供一套 **Clash Verge（Rev）/ Stash** 的推荐配置与分流策略
...
3. 慢速VPS节点，可能会导致iOS的VPN频繁崩溃
   在iOS上，如果你的VPS节点过于慢，可能也会导致NS查询时，内存爆炸（超过iOS的50MB限制），从而导致VPN崩溃。如果遇到这种情况，请将DNS查询指定到更换更快的VPS（哪怕带宽低都不怕，只要速度响应快）。

对于DNS，要特别注意：

- 本模版的DNS请求，采用白名单制度，只有命中了的中国域名，才会使用操作系统内置的的DNS进行请求，其他域名的DNS请求，均通过VPS进行代理，并且在此过程中，不会通过系统内置的DNS请求功能向国内DNS发起请求。从而最大限度的避免DNS泄漏。并且也没有Fake IP，所有DNS请求，只请求一次，只返回1.1.1.1/8.8.8.8解析后的真实IP。
...
## 许可与声明
- 本代码使用MIT协议分发
- 参与编写：OpenAI Chatgpt 5pro Agent
