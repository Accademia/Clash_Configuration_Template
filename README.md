...
建议步骤（最佳实践）：
  ...
  - 第四步（非必要），逐一检查，如下配置，是否有 微调需求 
      ...
      - 是否切换到 ：只支持 Clash Meta 模式❓ ｜ 启用方法，在源代码中查找：
      ...
      - 是否切换到 ：只支持 Stash 模式❓ ｜ 启用方法，在源代码中查找：
          
          - ClashMeta_Only 包裹着的代码 ，关闭 ❌
          
          - Stash_Compatible 包裹住的代码，启用 ✅

          - ⚠️ 默认配置 = 只能做到stash客户端不报错 + 翻墙基本功能正常工作。但默认配置下，链式代理 在stash客户端上，会 退化为节点直连（落地机）。必须做完上述改动，才能让链式代理在stash上生效！
            
      - 是否切换到 ：只使用 黑名单模式 ❓  ｜ 启用方法，在源代码中查找：
