...
- 无DNS泄漏 （ 海外域名，只发送给VPS解析，不会发送给国内DNS服务器。国内域名 只发送给 国内DNS，全程使用DOH加密访问）
- 最大化节约内存（ DNS请求只发送一次，全程关闭Fallback DNS ）
- 最小化电力消耗 （ 关闭所有 冗余并发 ）
- 国内主流APP的 IP归属地显示 可实时切换（到国外）
- 支持黑白名单模式 实时切换 （无需切换规则文件）
...
【PC端】：
推荐 规则2 + Adguard客户端。其中，Adguard用来防止隐私泄漏和屏蔽广告，效果比内置的规则更好用（因为可以实时更新）！
