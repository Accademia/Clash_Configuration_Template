...
<br>
 ...
 - 本模版，为了让🇨🇳中国网站，只会解析到🇨🇳国内IP。在 “DNS分流策略” 中，使用了白名单机制（geosite:cn）。让国内网站的域名，直连国内DNS，解析道国内IP地址。在获得🇨🇳国内IP地址后，又使用了geoip:cn 对域名解析后的IP进行分流。从而确保几乎是 99% 情况下，访问🇨🇳国内网站时 ，不会绕路到国外。
 
 - 而对于，不包含在上述白名单（geosite:cn）中的冷门网站，会不会绕路 ？ 您放心，越是冷门小众，100%越是用不起海外CDN（ 穷 + 核心受众在国内 ），或者压根不屑于用海外CDN（比如 医院 大学 政府）。在这种情况下，你在外国DNS请求域名解析，一样返回的是中国IP。只要是🇨🇳中国IP，就会通过geoip:cn分流到 “直连”。可以说， **geosite:cn + geoip:cn**  = 几乎99.99%覆盖 🇨🇳 == 0绕路。
 
 -  本模版，同时还给了 “微软、苹果、索尼、任天堂” 特别预设。对于海外公司，只默认这四个大厂，是直连模式（但是，其AI服务除外）。以便于， 避免 系统升级、软件升级、游戏升级 时，把VPN流量耗光 ❕❕❕

 - 如果 仍旧满足不了你的需求，可以将你认为发生绕路的 🇨🇳 国内域名，加入到本模版的 “ nameserver-policy ” 字段中。即可秒修复 绕路问题 。
