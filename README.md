...
DNS泄漏会导致 ，GFW 、公司网关、电信运营商的防火墙、校园网出口，通过你发出的DNS请求，窥探你都去了哪些网站。 危害极大。

 - 本模版的DNS请求，采用白名单制度，只有命中了的中国域名分流规则集的，才会使用🇨🇳国内的DNS（DOH加密访问）进行请求
 - 其他域名的DNS请求（非中国域名解析请求），均通过VPS进行代理（境外DOH加密访问）
 - 并且，本模版，也没有启用Fake IP，所有DNS请求，只请求一次。即，在请求境外DNS的过程中，不会向国内DNS发起请求。从而最大限度的避免DNS泄漏。
