...
- 最小化电力消耗 （ 关闭所有 冗余并发。精简浓缩 规则数量。并将日常场景中，高命中的规则放在最前部，避免出现 每次网络请求，都要对比了十几万条 才发现是 域名是日常使用的 🇨🇳国内IP。 ）
...
- 哪怕跟好友微信视频、看抖音快手、下载APP、收发邮件、各种国内办公软件、网盘，都不会因 本模版开启后常驻后台，而导致上网卡顿、绕路 或 消耗VPN节点流量。
...
- **完全隐秘行踪**：
    - 默认为白名单模式（出境流量 = 大部分 通过VPN节点代理 ）！在默认模式下，对GFW、上层网络出口，做到100%隐秘境外行踪。
...
#  本模版的 “白名单模式” ， 会不会 “绕路” 导致 大量消耗VPN流量 ？

<br>

不会❕❕❕

 - 本模版使用了 （魔改后的）geosite:cn 对DNS域名解析请求 进行分流，又使用了geoip:cn 对域名解析后的IP进行分流
     + 从而确保几乎是 99% 情况下，访问🇨🇳网站时 ，不会绕路到国外。
     + 而对于，不包含在geosite:cn中的冷门网站？您放心，越是冷门，100%越是用不起海外CDN（ 穷 + 核心受众在国内 ），或者压根不屑于用海外CDN（比如 医院 大学 政府）。在这种情况下，你在外国DNS请求域名解析，一样返回的是中国IP。只要是🇨🇳中国IP，就会通过geoip:cn分流到 “直连”。
     + 所以，本模版使用 **geosite:cn + geoip:cn**  = 几乎99.99%覆盖 🇨🇳 == 0绕路。
 
 -  本模版，同时还给了 “微软、苹果、索尼、任天堂” 特别预设。对于海外公司，只默认这四个大厂，是直连模式（但是，其AI服务除外）。以便于， 避免 系统升级、软件升级、游戏升级 时，把VPN流量耗光 ❕❕❕
...
+  也就是说，只要你有 🇺🇸 🇬🇧 🇦🇺 🇯🇵 住宅IP，你可以用本模版，全天在后台挂 这几个国家的任意金融级APP（如 花旗银行、劳埃德银行） ，也不会被账号封控（这些APP的GPS位置必须关闭）
...
具体行为如下：
 ...
 - 本模版的DNS请求，采用白名单制度，只有命中了 “中国域名规则集” 的，才会使用 🇨🇳国内的DNS（DOH加密访问）进行请求
 - 其他域名的DNS请求（对于非中国域名解析请求），均通过VPS进行代理（境外DOH加密访问）
 - 并且，全程不启用Fake IP ！即，所有DNS请求，只请求一次。即，通过境外VPS，代理境外DNS请求的过程中，不会向国内DNS发起请求。
 - 并且，无论国内外的DNS请求，全程都只使用DOH（DNS over Https）加密访问。
 - 并且，全程不使用 no-resolve（不限制域名解析请求）
 - 并且，通过 清洗 geosite:cn 规则集，将 geosite:cn 中的错误率降到几乎0%。（清洗后的规则集合如下）
     - https://github.com/Accademia/Additional_Rule_For_Clash/tree/main/GeositeCN
  
<br>

使用上述方案的代价是什么？会不会导致上网时，页面打开延迟增大。导致上网很卡？
...
# 本模版如何解决，白名单模式下，精准分流 “🇨🇳中国境内流量” 不绕路的 ？

<br>

本模版目前，的确已经实现了，几乎0绕路。 实现过程如下：
...
由于本模版，是白名单模式，并且，中国区分流的匹配，被安排到了的境外流量分流之前。所以，本模版对于🇨🇳中国区的规则集合，其 容错度极低。必须要求 中国区规则集合 要非常精准才行。而且必须精准到，宁可错漏不能错杀。但目前市面上，没有可用的高精准规则集合。
...
如果你日常工作需求中，必须要使用大而全（14万条规则）的 ChinaMax （ https://github.com/blackmatrix7/ios_rule_script/tree/master/rule/Surge/ChinaMax ）给中国网站分流。不然会对工作生活产生很大干扰。

那你可以 手动修改模版 。有两种 修改方案：

  - 方案 1 ：将“回国分流”从第二级（序号：2st），搬移到 第四级（序号：4st） 和 第五级（序号：5st）之间。

  - 方案 2 ：如果想将 ChinaMax 错误规则的影响降低最低（降到几乎忽略不计），可以将 “回国分流”从第二级（序号：2st），搬移到 第八级（序号：8st） 和 第九级（序号：9st）之间。也就是搬移到 兜底规则之前。
...
上述这样做的代价：
  ...
  - 占日常90%的流量的网络请求，即，最日常使用的 直连国内的网络请求，其优先级最低。导致，每次直连，都要先比对十几万条前置规则，才能发现命中的是直连国内🇨🇳。耗电大大增加！！
...
1. **不要将自己的节点借给别人**。 要知道, ”跨异地城市“的多设备 连接境外 “同一个VPS”，会被GFW重点监控。最好只在自己同城家庭内部  分享节点。
    ...
    - 节点不外借，就像车不外借一样。你放心，你借出去的节点，被用坏了，要修要处理的人、被麻烦的人，只会是你自己。对方绝对不会给你出一分钱、一份力，而且大概率还会心里埋怨你，为什么你的节点不好用。你是不是故意给弄坏不让人家用的。 🤣🤣🤣🤣 如果人家用你的节点，导致人家外国银行账号被封控，或者密码被窃取。那你就等着领骂吧。你们要闹掰了，人家把你给的VPN，举报到🚨，您100%被训诫，没准还喜提 “治安处罚“， 再帮👮完成 ”500软妹币的KPI”。虽然不入刑，但 以后无犯罪记录证明 ，100% 不是白纸。而且 您此后就被辖区挂上号 ， 喜提 活佛 大好人 ❕❕
