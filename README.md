...
- 双外部规则源 互为备份 （哪怕任意一个外部规则源 更新慢，也不怕）
...
- 强烈推荐 白名单模式：因为，足够省心！毕竟黑名单最大的问题，就是可能会三天两头 补充规则，不胜其烦。
- 白名单模式的代价： 需要 1TB以上月流量的 高质量节点 （关于 如何避坑 底质量节点，请看本页面的末尾—）
...
- 要省电是因为，在iPhone上，往往的VPN软件电量消耗，占整机电量消耗的20%（VPN永驻后台不关闭）
  ...
  + 所以，在本模版规则中，一般用户，被使用最频繁的国内分流（回国分流），被安排在（在广告后的）最前面。而是非大负载的海外规则，被安排在了国内分流之后。杜绝要匹配上万条规则，才发现是国内直连的情况。
...
## 如何魔改本模版，从而做到 进一步省电？
   ...
   - 本套规则中，为什么在同一个分流策略上，同时 冗余部署了 Ruleset + Geostie 两套规则 ？
       + 这是因为，通过冗余规则，避免因某个规则库停止更新（或更新慢），而导致，使用者频繁手动维护yaml。为了能省心使用，尽量做到一次书写，后期0维护。
