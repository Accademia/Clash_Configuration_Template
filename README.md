...
- 完美适配 全球各家AI（ 包括 Apple Intelligence ） 
...
- **分流 回国流量**：
    - 中国直连 = 几乎0绕路！
    - 回国场景 = 支持 以下每个国产APP都单独分流（且不影响IP归属地显示）
...
| 02-WhiteList-Min.AntiAD_模版.yaml | 2 万条  | 1.34 万条 | 0.65 万条 | 
| 03-WhiteList-Non.AntiAD_模版.yaml | 1.8 万条 | 1.3 万条 | 0.5 万条| 
...
本模版，默认是 白名单模式。使用过程中可以进行，黑白名单模式 实时切换。

你也可以通过手动魔改 本模版，切换到 只支持 **黑名单模式**
...
本模版的 白名单模式 ， 会不会扰路，导致消光VPN流量 ？

 - 不会！ 本模版使用了 （魔改后的）geosite:cn 对DNS域名解析请求 进行分流，又使用了geoip:cn 对域名解析后的IP进行分流，从而确保几乎是100%国内的网络访问 ，只会请求国内的CDN服务器。只会在极个别、极个别、极个别 的情况出现问题。我本人已经使用本模版 > 1年，手机 > 1000个国内外APP，样本已经比较充足了，完全无感。

<br>

本模版的 白名单模式 ，会不会 导致 系统升级（iOS/Windows）、AppStore升级 时，把VPN流量耗光？

 - 不会！ 本模版给了 微软、苹果 特别预设。海外公司，只默认这两个大厂，是直连模式（但是，其AI服务除外）
 
 - 其他的海外公司，你也可以通过预设的分流选项，手动改成直连模式。
...
DNS泄漏会导致 ，GFW 、公司网关、电信运营商的防火墙、校园网出口，通过DNS请求窥探你都去了哪些网站。 危害极大。

 - 本模版的DNS请求，采用白名单制度，只有命中了的中国域名分流规则集的，才会使用🇨🇳国内的DNS（DOH加密访问）进行请求
 - 其他域名的DNS请求，均通过VPS进行代理（境外DOH加密访问），在请求境外DNS的过程中，不会向国内DNS发起请求。从而最大限度的避免DNS泄漏。
 - 并且，本模版，也没有启用Fake IP，所有DNS请求，只请求一次。
 - 并且，无论国内外的DNS请求，全程只使用DOH（DNS over Https）加密访问。
  
<br>

使用上述方案的代价是什么？会不会因为境外域名的DNS，都走代理转发，而导致上网时，页面打开延迟增大。导致上网很卡？
...
#  本模版的DNS分流策略，会不会导致绕路（解析到国外IP）？

不会 ❕  

在本模版内：
  - 通过DNS分流策略，让中国域名 只向中国DNS 请求解析。从而国内CDN IP，实现 0境外绕路。

为了实现上述目标。本模版的DNS分流中，并没有 geosite:cn分流规则，因为这个规则集的精度太低。错误和冗余规则，高达2000多条，占其集合内规则总数量的29%。 会导致错误的DNS分流。
