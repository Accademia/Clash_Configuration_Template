...
对于 Stash用户，可以通过 手动修改 《 通用模版 》实现黑名单模式 ，只需要在配置模版中，进行如下两步之后，你就会得到一个 仅支持 黑名单模式 的模版：
...
2. ##### Clash模版 ：需要定制过滤器，来适配你的机场风味！
   ...
   由于每一家机场，对节点命名规则不同，所以，如何通过过滤节点命名，来归纳节点到不同的代理组，是需要针对每家机场进行适配的（而且这些命名往往是冲突的）。
   
   > - 如果你自己不会改，你可以问机场主，要filter的表达式。或者让机场主 帮你修改本模版，毕竟它收了你钱，这就是他应该干的活。
   >
   > - 如果机场主 只收钱不干活吃相难看。你也不要在这里发issue，我 ❌拒绝回答❌ 一般性问题，会直接关闭issue。建议你直接找付费AI去问（如 ChatGPT Think、Pro）。 注意一定是 💰 “付费AI” ，所有 “免费AI” 都是垃圾，一定会改错。
   > 
   > - 如果有增强适配性的更好的表达式（并且不跟现在的规则冲突、不跟主流情况冲突），可以反馈issue。
   ...
   注意： 本模版会假定，修改APP的IP归属地落地现实的节点，无需中转 本身就是最优质的线路。
   ...
   注意： 本模版会假定，所有 极品住宅IP的线路 都是非优质线路，都需要中转。
    
    >   - 现实也是如此，真实的 🇺🇸美国的车库机房，都是在真实住宅 + 真实的美国运营商（如AT&T）宽带组成的，价格都在几百人民币每月，别说国际出口差，连本地出口都很差，一般都是100Mbit 带宽 
