...
⚠️⚠️ 不看完 本说明自述，尤其不看完如下章节 《 如何 使用本模版》 《如何将 VPN节点，加入到本模版，并导入VPN软件》，就想指挥别人当保姆的 、就说本模版有BUG的。🤣 直接 0 社交 关闭issue⚠️⚠️

本模版 = 7x24挂机 高强度 自用级验证， 实锤 = 100% 可用，如果你分流失败，建议多照照自己的原因，实在不行 问ai。但肯定不是本模版有bug。

特别提示：本模版必须下载后，手工填入你自己的节点，才可使用！
