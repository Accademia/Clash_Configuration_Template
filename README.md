...
## 如何魔改本模版，从而做到 进一步省电？
   ...
   - 对于模版文件内，同时有 Ruleset规则 和 Geostie 规则的 分流选项上，删除Ruleset规则（ 并且删除其对应的rule-providers ）。这样可以将外部规则数量减少到1000个以下，同时，几乎不会降低分流能力。从而进步一省电。也就是说，这两者是互为冗余部署的。
     
   - 本套规则中，为什么在同一个分流策略上，同时 冗余部署了 Ruleset + Geostie 两套规则 ？ 通过冗余规则，避免因某个规则库停止更新（或更新慢），而导致需要频繁手动调整yaml。尽量做到一次书写，后期0维护。
