...
本模版 = 7x24挂机 高强度 自用级验证， 实锤 = 100% 可用，如果你分流失败，建议多照照自己的原因，实在不行 问ai。但肯定不是本模版有bug。（千万别问 免费AI、国产AI ！至少 Gemini 3 Pro + GhatGPT Think 多个 顶级付费AI 相互印证！ 再次强调，付费AI 付费AI 付费AI ❕ 不要听免费AI的 ）

特别提示： 本模版可 无脑导入。但必须 手工填入你自己的节点，才可正常使用！把本模版 当成免手工配置 + 自动筛选 订阅节点 的 🚫 100%不生效 🚫 

 > 之所以不支持自动筛选，这是由于 自动筛选 ，无法区分 哪些是落地节点、那些是中转节点，而本模版的 “抗节点损毁+IP稳定功能” 需要 精细区分上述特性，以便组成代理链，实现全自动切换。目前无解。未来，这需要ClashMeta改进，以便能在订阅节点声明时，带有更多ClashMeta官方预设的规格化的数据标签（如 RelayNode 、HomeIP 、US 等 ），从而完成节点筛选，而不是把所有筛选压力都堆在节点名称上 🤮 这简直就是 💩山架构

❌ 拒绝适配 ：除 Clash Meta、Stash 以外的 其他客户端
...
<br>
  ...
  - #### 模版 01 ：[通用模版]-WhiteList-01.yaml  
    ...
    >
    >  - 支持 黑明单翻墙 / 白名单翻墙 ，两种模式实时切换 （ = 默认启用 0.7万条 GFWlist规则 ） 
...
<br>
  ...
  - #### 模版 02 ：[通用模版]-WhiteList-02-Min.AntiAD.yaml 
     ...
     > 
     >    + 缩减 RuleSet 规则 ：将 “GFWlist” ，从0.7万条 精简到 0 条 (裁剪后，不再支持 实时切换 到 黑名单模式)
     > 
...
<br>
  ...
  #### 注意：以下模版，在上述 “适配桌面端 的 模版” 基础上，又又再次裁剪、阉割（ 0添加 新代码 ）。进一步 额外阉割了 “实时切换黑名单模式” 功能 ， 和 其他冗余规则 （用以优化功耗。极致省电 🔥 ）

| 📱 适配移动端  | 总规则数 | RuleSet规则数 | GeoSite规则数 |
|---|---|---|---|
| [ClashMeta模版]-[移动端]-WhiteList-01.yaml  | 12.5 万条 |  9.8 万条 | 2.7 万条 | 
| [ClashMeta模版]-[移动端]-WhiteList-02-Min.AntiAD.yaml 🔥🔥🔥  | 1.9 万条  | 1.2 万条 | 0.7 万条 | 
| [ClashMeta模版]-[移动端]-WhiteList-03-Non.AntiAD.yaml | 1.7 万条 | 1.2 万条 | 0.5 万条|
...
所以，你下载 [通用模版] 后，你需要更多步骤 的 手工配置才能用起来（可配置性更全面）。所以，其代码容量更大。
 ...
 - #### [ClashMeta模版] 
   ...
   - #### [ClashMeta模版]-[桌面端]
     ...
     >
     >   - 启用了 ： GFWlist 规则   - 用于 实时切换 “黑白名单模式” （ 关闭后，无法 “实时切换” 到黑名单模式 ）
   ...
   - #### [ClashMeta模版]-[移动端] 
     ...
     >
     >   - 效果：只使用 桌面端 10% 的规则数量，达到了桌面端 98% 的分流效果。
...
<br>
 ...
 - ### 📱 手机、平板、路由器  
   ...
   - #### 为什么不选 01 模版？
     > 
     > “模版 01” 虽然更全面，但是在移动端会极大消耗电力（规则数量膨胀了10倍），续航➗2。
     >
...
<br>
 ...
 - ###  💻个人电脑  
   ...
   - #### 首选 :  ✅ [ClashMeta模版]-[桌面端]-03 ✅  +  Adguard 客户端
     
     > 规则数量 ：12 万条
     >
     > 入选理由 ：无 （ 中国分流规则 额外增加10万条 ）
 
   - #### 次选 :  ✅ [ClashMeta模版]-[移动端]-03 ✅  +  Adguard 客户端
...
### 翻墙模式 ？
 ...
 - WhiteList + 移动端 ：只支持 白名单模式 ❕ 🔥 （ 推荐使用 ！ ）
 
 - WhiteList + 桌面端 ：默认 白名单模式 、可切换至 黑名单模式❕

 - BlackList : 只支持 黑名单模式 翻墙
...
<br>
  ...
  - ### 第五步（非必要），中国分流增强（防绕路海外）
      ...
      - 如果 你是桌面端设备（非手机） ，且 你访问的 中国网站 发生了 绕路海外VPN的情况，可以考虑启用 ChinaMax 规则
...
4. ### 落地节点 更多的坑
    ...
    要测试IP的干净程度，最好是双ISP 家庭宽带IP、住宅IP 或者 移动网络IP。但还要检测其国际出口 或落地国的出口是否足够大。大部分这种IP的节点，出口都很小，50M或者200M顶天了。而且。如果真的是 “美国的车库级机房” 你还可能会遭遇 频繁断流。坑比较多。
