...
-  ### **保证IP地址稳定** 
   ...
   > 通过  **代理链** ，锁定落地节点，中间的中转节点，随便被墙，配置会自动选择最优线路（延迟最低的中转线路）。
...
建议步骤（最佳实践）：  
  ...
  - 第一步： 首次验证时，只使用一个VPN节点，将配置信息，复制到以上六个节点中（注意，只做填空，不要修改节点的name 。观察 ：是否能跑通（ = 境外翻墙成功 + 境内直连 ）

    <br> 
    
  - 第二步，如果上一步 验证通过，再将不同国家的VPN节点，配置到上面的六个节点上。观察是否能让，不同APP、不同国家的网站（比如 不同国家 的 显示IP网站），有不同的分流。
  ...
  - 第三步，如果前两步 已经验证通过，再根据你的节点信息，对 “ 节点名称 + 中转线路名称 ” 做 批量改名（只改name字段，不改其他配置）。
  
    <br> 
    
  - 第四步 ，导入到 Clash Meta 或 Stash 前的必备工作 
      
      - 在导入到 Clash Meta 客户端 之前， 请在源代码中查找：
    ...
    <br> 
      ...
      - 在导入到 Stash for iPhone 客户端 之前， 在源代码中查找：
...
<br>
  ...
  - 第五步（非必要），如果 你是桌面端设备（非手机） ，且 你访问的 中国网站 发生了 绕路海外VPN的情况，可以考虑启用 ChinaMax 规则
      ...
      - ⚠️ 注意：仅桌面端，可以考虑开启 第五步。而在移动端，尤其是在iphone，强烈不建议开启 第五步 。因为开启第五步后，规则数量 会暴增10倍，从1.2万条规则，膨胀到12万条。会导致 触发 iOS 50MB网络内存限制的概率增加，从而导致VPN软件崩溃。而且手机耗电也会大大增加！！
         
<br>
   
  - 第六步（非必要），如果 你嫌 VPN流量贵 ，可以切换到 “仅支持” 黑名单模式
...
只要你家局域网中，内置了 “反诈插件” 的 WIFI设备， 跑几个简单的 预设名单，就能拿到你在路由器上配置的 VPN节点IP，然后你的所有VPN IP都会被阻断。🤣🤣 纯纯的 0风险意识。

很多人会说，我家没断网，我一直用的很好，我不觉得有问题。🤣 纯0风险意识。GFW要这么部署，不比 深度包检测 效果好1000倍 ，你说人家动力有多大 🤣 简直就是分分钟定位所有VPN IP的黑科技（对于非链式代理）。

而且目前。国产路由器、国产光猫已经内置反诈插件了。下一步，你家的IOT设备，尤其是华为设备，必然部署只是时间问题。未来，所有IOT设备，都可能会通过OTA升级，实现内置反诈插件。像联通赠送的家庭摄像头、国产智能电视机、扫地机器人、智能中继、甚至你家智能冰箱。这些设备都可以实现上述效果。风险极大。尤其像智能电视这种，其风险更大。这种可安装第三方APP的设备，非常容易被病毒程序攻击 远程安装病毒APP。就像你用电视助手给你家电视装APP一样。其实，很多人家里的智能电视，都已经被入侵，在必要时会成为DDOS源，在平时，你家电视也可能会间歇性的发送垃圾邮件（只是消耗流量极低，你在路由器流量监控中根本看不出来有问题）。这些都是99.999%的人观察不到的。包括我朋友家的索尼9系（顶配）电视机，依旧会中毒，而且反复重置都不管用（隔一段时间又会被感染）。如果你不是专业级的，根本察觉不出来，因为病毒几个小时、几十分钟才会不定期才发送一批的垃圾邮件。这些病毒，在发送垃圾邮件的过程中，如果匹配到了你部署在OpenClash路由器上的的规则，如果使用了你的VPN节点发垃圾邮件，会导致你的节点瞬间被封禁。即便你是订阅节点不怕被封，那你也可能会频繁切中转线路，体验极差。再进一步，如果这些设备，升级内置反诈插件，成为GFW部署在你家局域网内的探测器，那更会让你部署在路由器上的所有VPN节点同时报废 。所以，强烈 不不不不不不不建议，把所有节点和规则，都配置到家庭总出口的路由器上。
...
分层！ 必须以 “分层” 的方式，部署VPN ！才能，达到 最省心（0维护+抗损毁）的效果。 如下：
 ...
 - 第一层：路由器上，只使用GFWlist黑名单 + 1-2个节点。以便能实现基础的外网访问功能。 以便 规则损坏时可以正常上外网，以及智能电视可以访问Youtube。用尽量窄的规则和尽量少的节点，将 损毁代价 + 损毁风险 降到最低。
...
方案2 的优缺点：
  ...
  - 高频使用的 “直连国内🇨🇳规则” （ GeositeCN ）。 在 境外分流前的第一次排查中 被直接命中 （ 本模版中的 第二级规则 、2st ）。🔥 并且由于GeositeCN 被前置到分流规则的前部，从而避免了，像其他模版一样，要比对完所有境外规则（超过几万条规则），才发现是平时常用的直连的情况。从而大大降低了耗电❕❕❕
  
  - 低频使用的 “直连国内🇨🇳规则” （ ChinaMax ）。 只在兜底规则发挥作用前，仅做二次排查使用， （ 被安置在了 本模版中的 第九级规则 、9st ）。此时，精度低 也不怕，因为海外访问都已经分流走了。剩下的大概率是中国域名了、或非GFWlist敏感的国外域名了。此时，可以说 ，几乎能随便错，随便泄漏，都不怕。
  
  - 等于说，那 GeositeCN 做一级高速缓存 （高频 + 高精准度），那 ChinaMax 做二级缓存 （低频 + 精准度稍低），并且，特别注意 ⚠️ 上述两者，仅仅在 Nameserver-Policy DNS （DNS分流策略）中启用 ⚠️，并且 指向 🇨🇳 中国DNS。也就是说，通过Nameserver-Policy DNS（DNS分流策略），如果解析到的是中国IP，才会在后续的 Rule（分流规则）中，通过 geoip:cn 直连。通过上述 多级缓存 多层套娃，从而，最大化的清洗掉ChinaMax中的 个别错误数据。

     - 本模版引用的ChinaMax已经做过一次离线数据清理，删除掉了1.9万条没有任何中国IP的域名

  - 虽然，上述的两级缓存机制，避免了 “功耗爆炸” ，但是 “内存爆炸” 无法避免。由于iPhone （ iOS 26 ）目前只有 50MB vpn网络内存。所以，在iOS移动端，依旧不建议开启。桌面端、路由器，不怕费电的，则可以部署。
...
⚠️ 本模版引用的ChinaMax是经过清洗的（ 非原版 ChinaMax ），原版ChinaMax中域名规则 11.7万条，经过清洗，将纯外国IP的域名删除后，规则数量下降到 9.8万条。但是要特别注意， 对 ChinaMax 的清洗，没有对于GeositeCN精细，可能还有少量漏网之鱼。
