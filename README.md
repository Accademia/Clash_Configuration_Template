...
| 02-WhiteList-Min.AntiAD_模版.yaml | 1.9万条  | 1.2万条 | 0.7万条 | 与 规则1 的区别 ： RuleSet规则中， “反广告 + 隐私保护” ，从7.7万条 精简到 1500 条（ 基于Geosite ），以及 GeoSite规则中，“Porn类网站” 的规则，从 1.3万 精简到 仅十几条（pornhub）|
...
## 使用建议：
 ...
 - 【PC端】 ：  首选 规则03 + Adguard客户端
     - Adguard 用来防止隐私泄漏和屏蔽广告，效果 比规则1 更好用。因为可以 Adguard 实时更新，规则集也更全面。（注意，Adguard 正版是 “买断制付费” ）
     - 下一章会解释，为什么 将 “反广告+隐私保护”，交给Adguard，远比使用Clash规则要好。
        

.

## 本套模版 最大特色是什么？如何利用这种特色？

  - 省电，是本套规则的灵魂 ❕❕❕❕
    
  - 在移动端，每次网络请求都要匹配10万条规则，是完全没有必要的，用1500条规则的反广告就可以解决90%广告问题。从而换来，一倍以上的续航提升，是非常值得的。（iOS还有50MB网络内存限制，在规则上做“减法”，也是最佳选择 ）
    
  - 在PC端，无论Mac还是Win，更可以通过Adguard，做到 “更灵活、更实时、更全面” 的 反广告 + 反隐私泄漏。就更没有必要在Clash规则中，自己堆砌规则数量了。从而保证，最低频度的手动切换VPN状态。达到全程VPN 无感上网。
      + 经过长期实测，Adguard客户端（买断制付费版）的 误拦截，几乎是0，在MacOS上使用了一年多，一次误拦截都没碰上。而 blackmatrix7 的 Clash 反广告 隐私保护规则的误拦截，碰见了十几次，为此还做了 修复规则。
...
## 如何魔改本模版，从而做到 进一步省电？
   ...
   - 对于模版文件内，同时有 Ruleset规则 和 Geostie 规则的 分流选项上，删除Ruleset规则（ 并且删除其对应的rule-providers ）。
       + 这样可以将，外部规则数量➗2，同时，几乎不会降低分流能力。从而进步一省电。也就是说，这两者是互为冗余部署的。
