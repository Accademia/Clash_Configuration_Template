...
不会 ❕ 本模版的DNS策略 已实现 0 DNS泄漏 ❕❕❕

0 DNS泄漏 = 完美的保护隐私。 防止GFW 、公司网关、电信运营商的防火墙、校园网出口，通过DNS请求窥探你都去了哪些网站。 
...
在本模版内：
  - 非中国大陆域名，不将域名请求发送给中国的DNS，以便最大化避免DNS泄漏 ！
  - 同时，中国域名，也只会向中国DNS，请求域名解析。从而解析到国内CDN的IP。从而实现，0境外绕路。

为了实现上述目标。本模版的DNS分流中，没有 geosite:cn分流规则，因为这个规则集的精度太低。错误和冗余规则，高达2000多条，占其集合内规则总数量的29%。 会导致错误的DNS分流。

为了解决无规则集可用的情况，我临时做了针对geosite:cn修复错误后的规则:
  - https://github.com/Accademia/Additional_Rule_For_Clash/tree/main/GeositeCN
  - 将原本geosite:cn中6800条规则，进行清洗后，保留了4800条 100% 准确的规则。用于将DNS分流的精准度拉满。
